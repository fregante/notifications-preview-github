...
import { extensionOptions, type ExtensionOptions } from '~libs/options-storage';
...
setFetch(
    // @ts-expect-error no content definition
...
async function updateUnreadCount() {
    ...
    updateNotificationsIndicatorStatus(Boolean(latestCount) && Number(latestCount) + rghCount > 0);
    ...
    for (const statusElement of $$('.notification-indicator .mail-status')) {
        ...
        statusElement.classList.toggle(
            'unread',
            Boolean(rghCount) || latestStatusElement?.classList.contains('unread'),
        );
    ...
    for (const indicator of indicators) {
        ...
        indicator.parentElement?.prepend(
            <details className="NPG-container details-overlay details-reset" onClick={onClick}>
                ...
                {/* @ts-expect-error need custom react element */}
                <details-menu
    ...
    if (!isOpen(dropdown) && list.length > 0) {
        ...
        delegate('.NPG-dropdown button', 'click', async (event) => {
            ...
            if (notification) {
                ...
                if (group && $$('.js-notifications-list-item', group).length === 0) {
                    group?.remove();
                }
            } else {
                ...
                if (group) {
                    ...
                    form.append(<label>&nbsp;Marked {notifs.length} notifications as read</label>);
